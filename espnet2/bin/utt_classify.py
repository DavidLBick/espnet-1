...
class Speech2Text:
    ...
    def __call__(
        ...
        Tuple[Optional[str], List[str], List[int], List[float], Optional[torch.Tensor]]
    ]:
        """Inference
        ...
        results, att = self.decoder(enc, enc_lens)
...
def inference(
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            for key in keys:
                # key = keys[0]
                for n, (text, token, token_int, score, att_wt) in zip(
